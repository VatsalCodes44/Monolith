import React from "react";
import Svg, { Path, SvgProps } from "react-native-svg";

export const WB: React.FC<SvgProps> = (props) => {
  return (
    <Svg viewBox="0 0 81 81" {...props}>
      <Path d="M0 0h81v81H0z" fill="none" />
      <Path
        d="m61.4 71-11.9-9.1 10.6-14.6c0-11-4.9-19.9-11.9-26.3l-1.7-.8-7.9 14.3-1.5-1.6 7.8-14.7 2.7-5.1v-.2c0-3.6-2.9-6.6-6.6-6.6-3.6 0-6.6 2.9-6.6 6.6v.2l2.3 4.4C28 24.1 20.9 34.1 20.9 47.3l10.6 14.6L19.6 71h41.8z"
        fill="#fff"
      />
      <Path
        d="M45.6 70.7H60l-10.4-9.5h-6.8zm5.2-45.6C54.7 42.1 36 55.8 36 55.8l7.3 4.7h7.4l10.2-13.2-1.5-8.6-3.6-8.3-2.6-4.4-2.4-.9zM44.5 8.6c.8 2.4-3.4 4.4-3.4 4.4v5.1l3.3.1 3-4.3-.3-3.4L45 8.6h-.5z"
        fill="#cfcecf"
      />
      <Path d="M65 72.2 50.9 61.6l10.2-14v-.4c0-14.1-7.6-23.1-12.2-27.3l-2.3-2.3 2.1-3.9.1-.3v-.5c0-4.3-3.5-7.9-7.9-7.9-4.3 0-7.8 3.5-7.8 7.9v.5l1.9 3.7c-5.9 4.6-15.5 14.6-15.5 30.1v.4l10.2 14L16 72.2h49zM22.5 46.9c-.3-14.6 9.3-24 15-28.2l.9-.6-2.7-5.1c0-2.9 2.4-5.3 5.3-5.3s5.3 2.4 5.4 5.3L35.8 34.6v.1l2.2 5.8 9.3-18.8 1.1 1c4.8 4.7 10.4 12.7 10.1 24.1l-9.2 13H31.8l-9.3-12.9zm10.4 15.6H48l9.8 7.3H23.2l9.7-7.3z" />
      <Path d="M36.2 18h4.9l-2.7 2.1h-3.8" />
    </Svg>
  );
};